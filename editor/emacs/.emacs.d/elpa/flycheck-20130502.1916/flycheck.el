...
;; flycheckにruby-lintを対応させるよ
(flycheck-declare-checker ruby-lint
  "A Ruby syntax checker using the Ruby-lint."
  :command '("ruby-lint" source)
  :error-patterns
  '(("^\\(?1:.*\\): warning: line \\(?2:[0-9]+\\), column \\(?3:[0-9]+\\): \\(?4:.*\\)$" warning)
    ("^\\(?1:.*\\): error: line \\(?2:[0-9]+\\), column \\(?3:[0-9]+\\): \\(?4:.*\\)$" error))
  :modes 'ruby-mode)
